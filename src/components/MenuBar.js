        ...
        return (
            <ul className="menu" style={{ backgroundColor: this.configColor.background }}>
                {elementsMenu.map((menu, index) => {
                    return <MenuItem key={index} menu={menu} elementsSubMenu={elementsSubMenu} itemColor={this.configColor.itemColor} subitemColor={this.configColor.itemBackground} activeColor={this.configColor.itemActive} />
                })}
            </ul>
        )
    }
